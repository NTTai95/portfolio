...
import { EditableCell } from "./edit-table-industries";
...
import {
  ...
  deleteIndustrie,
  updateIndustrie,
} from "@/api/industriesAPI";

const TableIndustries: React.FC = () => {
  const [form] = Form.useForm();
  const dataFromHook = useIndustrie(); // hook phải gọi trong body component
  ...
  const edit = (record: Partial<DataType> & { key: React.Key }) => {
    form.setFieldsValue({ name: "", description: "", status: "", ...record });
  ...
  const save = async (key: React.Key) => {
    try {
      ...
      if (index > -1) {
        ...
        await updateIndustrie(key as string, updatedItem);
  ...
  const deleteRecord = async (key: React.Key) => {
    try {
      await deleteIndustrie(key as string);
  ...
  const addNewRow = async () => {
    ...
    try {
      // Gửi request tạo mới
      const res = await createIndustrie(newRow);
    ...
    <div className="p-4">
      ...
      <Form form={form} component={false}>
        <Table<DataType>
          ...
          pagination={{ onChange: cancel, pageSize: 10 }}
        />
      </Form>
    </div>
  );
...
export default TableIndustries;
